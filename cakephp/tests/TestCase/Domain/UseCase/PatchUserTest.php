...
class PatchUserTest extends TestCase
{
    ...
    public function testInvoke(): void
    {
        ...
        // patchUser メソッドが1回呼ばれることを期待
        $usersServiceMock->expects($this->once())
            ->method('patchUser')
            ->willReturn($this->mockedUser);

        // UseCaseを作成
        $useCase = new PatchUser($usersServiceMock);

        // UseCaseを実行
        $result = $useCase('1', $this->mockedUser);
    ...
    public function testInvokeEmpty(): void
    {
        ...
        // patchUser メソッドが1回呼ばれ、空の配列を返すことを期待
        $usersServiceMock->expects($this->once())
            ->method('patchUser')
            ->willReturn([]);
        ...
        // UseCaseを実行
        $result = $useCase('2', $this->mockedUser);
